...
(function (global, factory) {
    ...
    avalon.directive('on', {
        ...
        parse: function (copy, src, binding) {
            ...
            var uuid = underline + '_' + binding.expr.replace(/\s/g, '').replace(/[^$a-z]/ig, function (e) {
    ...
    function parseExpr(binding) {
        ...
        replace(/(\b[\$\w]+\s*):/g, dig).replace(/\|(\w+)/g, function (a, b) {//移除所有过滤器的名字
            return '|' + dig(b)
    ...
    function collectLocal(str, local) {
        str.replace(/__vmodel__/, ' ').replace(robjectProperty, ' ').replace(rvar, function (el) {
